...
public class Calculator {
    	...
    	public double modeArray(double[] array) {
    		 if (array.length == 0) {
    	        ...
    	        // Create a frequency map to store the occurrences of each element
    	        ...
    	        for (Map.Entry<Double, Integer> entry : frequencyMap.entrySet()) {
    	            if (entry.getValue() == maxFrequency && entry.getKey() != mode) {
    	                throw new IllegalStateException("Array has multiple modes");
    	            }
    	        }
    	...
    	public double varianceArray(double[] array) {
    		...
    		double sumSquaredDifferences = Arrays.stream(array).map(x -> Math.pow(x - mean, 2)).sum();
